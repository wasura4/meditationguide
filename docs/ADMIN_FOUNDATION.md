...
The admin foundation and daily editorial workspace shipped through PR #7 on 2026-09-24. Production commit `d4529a9e565a3c81397f62096b349845577d0f2f` is live at https://meditationguide.vercel.app. Matching Firestore rules were deployed to `nirvanaya-web` and their deployed content was verified.
...
This branch does not implement trusted account suspension, role-management UI, Storage rule hardening, asset cleanup, a system-wide audit log, review/scheduling, an inbox. Existing public reads for audio and metric collections retain their previous behavior; draft visibility is restricted for Dhamma posts. Other collections still need comprehensive schema validation. Full collection reads remain in trend analytics, name/email search, playlist management/track selection and archive dependency checks; maintained summaries, reverse-reference indexes and indexed full-text search are scaling follow-ups. Library text search currently covers loaded records, clearly labeled in the UI.

The subsequent learning stage is documented in [LEARNING_PATHS.md](LEARNING_PATHS.md). Review/scheduling and operational automation follow it.
