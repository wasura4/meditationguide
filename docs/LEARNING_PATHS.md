...
The learning extension ships through PR #8 together with the Categories/Types theme fixes. Its additive Firestore rules were deployed to `nirvanaya-web` on 2026-09-24 (ruleset `422b2499-780c-4ba7-b71e-e8fd0394362a`), and read-back content exactly matched `firestore.rules`. The previous deployed rules were checked against the foundation release and backed up before deployment. A Vercel Git deployment alone does not install Firebase rules. After release, create verified teacher biographies and editorially reviewed paths through the admin UI. Nothing is prepublished automatically.

Categories and Types now use theme-aware surfaces, fields, tags and accessible scrollable dialogs, with wrapping mobile actions. Tailwind dark variants follow the app's selected theme class instead of independently following the operating system. Creating an inactive meditation type now respects the unchecked Active field. Local browser checks covered desktop and 390px layouts, light/dark lists and forms, and inactive type creation. Emulator development builds use `.next-emulator` to avoid bundle collisions with normal development servers.
