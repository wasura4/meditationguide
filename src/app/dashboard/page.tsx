...
import { DashboardSkeleton, ReadingSkeleton } from "@/components/dashboard/DashboardSkeleton";
...
function DashboardContent() {
  ...
  useEffect(() => {
    ...
    async function refresh(showLoading = false) {
      ...
      // Settle each section independently so a slow request cannot hide ready data.
      async function refreshSessions() {
        try {
          ...
          if (isCurrent()) setSessionsError(true);
        } finally {
      ...
      async function refreshPosts() {
        try {
          const result = await DhammaService.getPublishedPosts();
          if (!isCurrent()) return;
          setPosts(result.slice(0, 2));
          setPostsError(false);
        } catch {
          if (isCurrent()) setPostsError(true);
        } finally {
          if (isCurrent()) setPostsLoading(false);
        }
      }
      await Promise.all([refreshSessions(), refreshPosts()]);
    }
        ...
        <div className="home-content space-y-6 pb-8">
          ...
          <section aria-labelledby="home-reading-title">
            ...
            <div className="home-glass divide-y divide-border/60 overflow-hidden">
              {postsLoading ? <ReadingSkeleton /> : postsError ? <div role="alert" className="p-5 text-sm"><p>{t("interface.content_error")}</p><button onClick={() => setAttempt(value => value + 1)} className="min-h-11 underline">{t("common.retry")}</button></div> : posts.length ? posts.map(post => <Link key={post.id} href={`/dhamma/${post.id}`} className="app-row">
...
export default function DashboardPage() {
  const { user } = useAuth();
  return <ProtectedRoute loadingFallback={<DashboardSkeleton />}><DashboardContent key={user?.id || "signed-out"} /></ProtectedRoute>;
}
