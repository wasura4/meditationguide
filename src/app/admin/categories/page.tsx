...
export default function AdminMeditationCategoriesPage() {
  ...
  const handleCreateCategory = async (formData: FormData) => {
    ...
    try {
      ...
      await MeditationCategoryService.createCategory(
        categoryFormData,
        adminUser.id,
      );
      showToast({
  ...
  const handleDeleteCategory = async (categoryId: string) => {
    ...
    if (isInUse) {
      showToast({
        ...
        title: "Cannot Delete",
        message:
          "This category is being used by meditation types. Please reassign them first.",
        duration: 5000,
      });
      return;
    }

    if (!confirm("Are you sure you want to delete this meditation category?"))
      return;
  ...
  const handleToggleStatus = async (
    ...
    if (!hasPermission("content", "update")) return;
    try {
      await MeditationCategoryService.toggleActiveStatus(
        categoryId,
        !currentStatus,
      );
      showToast({
            ...
            <AdminDialog
              title={
                editingCategory
                  ...
                  : "Create New Meditation Category"
              }
              onClose={() => {
                setShowCreateForm(false);
                setEditingCategory(null);
              ...
              >
                ...
                <div>
                  ...
                  <div className="grid grid-cols-5 gap-3 sm:grid-cols-7">
                    {AVAILABLE_COLORS.map((color) => (
                      <button
                        ...
                        className={`h-11 w-11 rounded-xl ${color.class} transition-all ${
                  ...
                  <p className="text-xs text-muted-foreground mt-2">
                    Selected:{" "}
                    {
                      AVAILABLE_COLORS.find((c) => c.name === selectedColor)
                        ?.label
                    }
                  </p>
                </div>
            ...
            <div className="divide-y divide-border">
              {categories.length === 0 ? (
                ...
                categories.map((category) => {
                  const colorClass =
                    AVAILABLE_COLORS.find((c) => c.name === category.color)
                      ?.class || "bg-gray-500";
                  return (
                    <div
                      key={category.id}
                    ...
                    >
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div className="min-w-0 flex-1 flex items-start gap-4">
                          <div
                            className={`w-12 h-12 rounded-lg ${colorClass} flex-shrink-0`}
                          ></div>
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center space-x-3">
                              <div
                                className={`w-3 h-3 shrink-0 rounded-full ${category.isActive ? "bg-green-500" : "bg-gray-400"}`}
                              ></div>
                              <div>
